...
//import DxfViewerWorker from "worker-loader" //"worker-loader!./DxfViewerWorker"

/** Events: all DxfViewer supported events (see DxfViewer.Subscribe()), prefixed with "dxf-". */
export default {
    name: "DxfViewer",

    emits:['updateImgUrl','dxf-loaded', "dxf-cleared", "dxf-destroyed", "dxf-resized", "dxf-pointerdown",
                                 "dxf-pointerup", "dxf-viewChanged", "dxf-message"],
    ...
    methods: {
        async Load(url) {
            ...
            if(this.dxfViewer. HasRenderer() )
            {
                let dxfbounds = this.dxfViewer.GetBounds()
                let dxfsize_x = dxfbounds.maxX-dxfbounds.minX
                let dxfsize_y = dxfbounds.maxY-dxfbounds.minY
                this.$emit('updateImgUrl',this.$refs.canvasContainer, dxfsize_x, dxfsize_y, this.dxfViewer.GetDxf())
    ...
    mounted() {
        this.options.canvasWidth = this.canvasSize.Width
        this.options.canvasHeight = this.canvasSize.Height
        ...
        if(this.$props.dxfUrl!==null)
        {
            this.Load(this.$props.dxfUrl)
        }
    },
