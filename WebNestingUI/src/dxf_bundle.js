...
class Arc extends DatabaseObject {
    ...
    tags(manager) {
        ...
        if (this.colorNumber) manager.push(62, this.colorNumber);
        else if (this.trueColor && this.trueColor !== -1) manager.push(420, this.trueColor);
    }
...
class Text extends DatabaseObject {
    ...
    tags(manager) {
        ...
        if (this.rotation) manager.push(50, this.rotation);

        if (this.colorNumber) manager.push(62, this.colorNumber);
        else if (this.trueColor && this.trueColor !== -1) manager.push(420, this.trueColor);
        
        if (this.style) manager.push(7, this.style);
...
module.exports = Text;
...
},{"./DatabaseObject":7}],27:[function(require,module,exports){
    const DatabaseObject = require("./DatabaseObject");
    ...
    class MText extends DatabaseObject {
         ...
         * @param {number} x - x
         * @param {number} y - y
         * @param {number} height - MText height
         * @param {number} width - MText width
         * @param {string} value - the MText string itself
         * @param {number} attachPt - [1-9] the MText attaching point 1=TL,2=TC,3=TR,4=ML,5=MC,6=MR,7=BL,8=BC,9=BR
         * @param {number} drawDir - the MText drawing direction 1=Left->Right 2=Top->Bottom 5= byStyle
         */
        constructor(
            x,
            y,
        ...
        tags(manager) {
            ...
            manager.point(this.x, this.y);
            manager.push(40, this.height);
            manager.push(41, this.width);
            manager.push(1, this.value);
            manager.push(71, this.attachPt);
            manager.push(72, this.drawDir);
            ...
            if (this.style) manager.push(7, this.style);
            if (this.spaceStyle) manager.push(73, this.spaceStyle);
            if (this.spaceFactor) manager.push(44, this.spaceFactor);
    
            manager.push(100, "AcDbMText");
        ...
        setSpacingStyle(spaceStyle) {
            this.spaceStyle = spaceStyle;
        }
        setSpacingFactor(spaceFactor) {
...
class TextStyle extends DatabaseObject {
    /**
     * @param {string} name - TextStyle table name
     * @param {string} [font = name] - font name
     * @param {string} [bigFont = ""] - bigFont name
     * @param {number} [obliqueAngle = 0] - oblique Angle
     * @param {number } [textGenerationFlag = 0] - text Generation Flag
     * @param {number } [widthFactor = 1] -width Factor
     * @param {number } [standardFlag = 0] -standard Flag
     * @param {number } [lastHeight = 1] -lastHeight
     * @param {number } [fixedTextHeight=0] -fixedTextHeight
     */
    constructor(
        name,
        font = name,
    ...
    tags(manager) {
        ...
        manager.push(2, this.name);
        /* No flags set */
        manager.push(70, this.standardFlag);
        manager.push(40, this.fixedTextHeight);
        manager.push(41, this.widthFactor);

        manager.push(50, this.obliqueAngle);

        manager.push(71, this.textGenerationFlag);
        manager.push(42, this.lastHeight);

        manager.push(3, this.font);
        manager.push(4, this.bigFont);

        if(this.xData){manager.push(1001, this.xData);}
        if(this.xFont){manager.push(1000, this.xFont);}

    }


    setFont(font){this.font = font}
    setBigFont(bigFont){this.bigFont = bigFont}
    setObliqueAngle(obliqueAngle){this.obliqueAngle = obliqueAngle}
    setTextGenerationFlag(textGenerationFlag){this.textGenerationFlag = textGenerationFlag}
    setWidthFactor(widthFactor){this.widthFactor = widthFactor}
    setStandardFlag(standardFlag){this.standardFlag = standardFlag}
    setLastHeight(lastHeight){this.lastHeight = lastHeight}
    setFixedTextHeight(fixedTextHeight){this.fixedTextHeight = fixedTextHeight}
    setXData(xData){this.xData = xData}
    setXFont(xFont){this.xFont = xFont}
}
...
},{"./AppId":1,"./Arc":2,"./Block":3,"./BlockRecord":4,"./Circle":5,"./Cylinder":6,"./Dictionary":8,"./DimStyleTable":9,"./Ellipse":10,"./Face":11,"./Handle":12,"./Layer":13,"./Line":14,"./Line3d":15,"./LineType":16,"./Point":17,"./Polyline":18,"./Polyline3d":19,"./Spline":20,"./Table":21,"./TagsManager":22,"./Text":23,"./TextStyle":24,"./Viewport":26,"./MText":27}]},{},[]);
